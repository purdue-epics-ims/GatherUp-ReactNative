...
- (void)dealloc {
    ...
    if (_track1_encrypted) {
        [_track1_encrypted release];
    }
    if (_track2_encrypted) {
        [_track2_encrypted release];
    }
    if (_track3_encrypted) {
...
-(void) verifyAndParse_encrypted:(NSData*) cardData {
    ...
    */
    ...
    self->_track1 = [tracks[0] retain];
    self->_track2 = [tracks[1] retain];
    self->_track3 = [tracks[2] retain];
    self->_track1_encrypted = [tracks_enc[0] retain];
    self->_track2_encrypted = [tracks_enc[1] retain];
    self->_track3_encrypted = [tracks_enc[2] retain];
    self->_serialNumber = [serialNumber retain];
    self->_KSN          = [ksn retain];
}
