...
- (void)viewDidLoad {
	...
	self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"About" 
        style:UIBarButtonItemStylePlain target:self action:@selector(goToAbout)];
...
- (void)umConnection_connected:(NSNotification *)notification {
    ...
    self.tabBar.selectedItem = [self.tabList_tbi objectAtIndex: 1];
...
- (void)tabBar:(UITabBar *)tabBar didSelectItem:(UITabBarItem *)item {
    ...
    if (tabIndex == self.tabList_tbi.count-1) {
        ...
        UIScrollView *scrollView =
            [[UIScrollView alloc] initWithFrame: self.vvRoot.bounds];
